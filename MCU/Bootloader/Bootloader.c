...
uint8_t auth_key[4] = {0x12, 0x34, 0x56, 0x77};  // 授权密钥
uint32_t flash_write_addr = APP_START_ADDR;       // Flash写入地址
...
void check_boot_condition(void) {
    ...
    uint32_t app_header = *(volatile uint32_t*)APP_START_ADDR;
            ...
            case BOOT_WAITING_FOR_AUTH:
                ...
                if (msg.id == 0x123 && msg.len == 4) {
                    if (memcmp(msg.data, auth_key, 4) == 0) {
                        ...
                        flash_erase(APP_START_ADDR, 0x10000);  // 擦除应用区域
...
void jump_to_application(void) {
    ...
    AppFunc app_entry = (AppFunc)*(volatile uint32_t*)(APP_START_ADDR + 4);
    // 设置栈顶指针
